...
function PostDetail() {
  ...
  return (
    <Container className="py-4" style={{ maxWidth: 760 }}>
      <Button variant='outline-secondary' size='sm' onClick={() => navigate(-1)} className='mb-3'>← Quay lại</Button>
      ...
      <Card className="shadow-sm">
        <Card.Body className="p-4">
          ...
          <p className='text-muted small mb-4'>✍️ {post.author} &nbsp;│&nbsp; 📅 {post.date}</p>
      ...
      <Row className='mt-4'>
        <Col className='text-start'>
          {prevPost && (
            <Button as={Link} to={`/posts/${prevPost.id}`} variant='outline-primary' size='sm'>← {prevPost.title}</Button>
          )}
        </Col>
        <Col className='text-end'>
          {nextPost && (
            <Button as={Link} to={`/posts/${nextPost.id}`} variant='outline-primary' size='sm'>{nextPost.title} →</Button>
          )}
        </Col>
      </Row>
    </Container>
